...
AST_OBJS = blocks.cmo parser.cmo scanner.cmo ast.cmo
PARSER_OBJS = blocks.cmo parser.cmo scanner.cmo ast_printer.cmo

default: clean markdown parser test
...
clean:
	rm -f markdown parser.ml parser.mli scanner.ml \
		*.cmo *.cmi *.out *.diff *.output parser ast
...
ast.cmo : scanner.cmo parser.cmi blocks.cmo
ast.cmx : scanner.cmx parser.cmx blocks.cmx
